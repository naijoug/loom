...
capture_theme() {
  ...
  capture "${prefix}new-task" "${BASE_URL}?screen=new-task&theme=${theme}" "新建任务|邀请 Agent 参与讨论|建议主 Agent|开始讨论"
  capture "${prefix}add-project" "${BASE_URL}?screen=add-project&theme=${theme}" "添加项目|打开本地目录|检测结果"
  capture "${prefix}session" "${BASE_URL}?screen=session&theme=${theme}" "Claude Code|子任务|进入测试验收"
  ...
  capture "${prefix}settings-general" "${BASE_URL}?screen=settings&tab=general&theme=${theme}" "当前项目|运行模式|项目技术栈"
  capture "${prefix}settings-appearance" "${BASE_URL}?screen=settings&tab=appearance&theme=${theme}" "主题|当前主题|字体与密度"
  capture "${prefix}settings-agents" "${BASE_URL}?screen=settings&tab=agents&theme=${theme}" "已安装 Agent|添加自定义 Agent"
  capture "${prefix}settings-safety" "${BASE_URL}?screen=settings&tab=safety&theme=${theme}" "项目终端槽位|执行护栏|日志敏感信息脱敏"
  capture "${prefix}settings-about" "${BASE_URL}?screen=settings&tab=about&theme=${theme}" "Loom|后端健康状态|诊断与支持|资源"
}
